...
namespace Better_Work_Tab.API
{
    ...
    public static class WorkGiverApi
    {
        ...
        public static bool CanPawnUseWorkGiverNow(Pawn pawn, WorkGiverDef workGiver, bool forced = false)
        {
            ...
            if (targetWorkType == null || PawnCompat.WorkSettings(pawn) == null)
            {
                return true;
            }

#if vAlpha4
            WorkGiverApiSnapshot alphaSnapshot = GetSnapshot(pawn, targetWorkType, workGiver, false);
            return alphaSnapshot.IsAvailableNow && !pawn.WorkTypeIsDisabled(targetWorkType);
#else
            ...
            return snapshot.IsAvailableNow &&
                   (PawnWorkControlCompatibility.NonColonistsCanDo(workGiver) ||
                    pawn.IsColonist ||
                    PawnWorkControlCompatibility.IsColonyMech(pawn) ||
                    PawnWorkControlCompatibility.IsColonySubhuman(pawn)) &&
                   !PawnWorkControlCompatibility.WorkTagsDisabled(pawn, workGiver) &&
        ...
        private static Pawn FindPawnByThingId(int pawnThingId)
        {
            ...
            return PawnsFinderCompat.AllMapsWorldAndTemporaryAlive.FirstOrDefault(pawn => pawn.thingIDNumber == pawnThingId);
        }
    }
}
