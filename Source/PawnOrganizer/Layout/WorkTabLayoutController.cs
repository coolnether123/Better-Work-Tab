...
namespace Better_Work_Tab.PawnOrganizer
{
    ...
    public class WorkTabLayoutController : IWorkTabLayoutController
    {
        ...
        private PawnDivider AddDividerAfterPawnWhileSorting(Pawn pawn, string label, Color color)
        {
            ...
            if (pawn?.playerSettings == null)
            {
                Log.Error($"[AddDividerAfterPawnWhileSorting] Pawn {PawnCompat.NameShortColored(pawn)} has no playerSettings!");
                return null;
            }
            ...
            if (visualIndex < 0)
            {
                Log.Error($"[AddDividerAfterPawnWhileSorting] Could not find pawn {PawnCompat.NameShortColored(pawn)} in current rows");
                return null;
            }

            //Log.Message($"[AddDividerAfterPawnWhileSorting] Pawn {PawnCompat.NameShortColored(pawn)} is at visual index {visualIndex}");
        ...
        private PawnDivider AddDividerBeforePawnWhileSorting(Pawn pawn, string label, Color color)
        {
            ...
            if (pawn?.playerSettings == null)
            {
                Log.Error($"[AddDividerBeforePawnWhileSorting] Pawn {PawnCompat.NameShortColored(pawn)} has no playerSettings!");
                return null;
            }
            ...
            if (visualIndex < 0)
            {
                Log.Error($"[AddDividerBeforePawnWhileSorting] Could not find pawn {PawnCompat.NameShortColored(pawn)} in current rows");
                return null;
            }
        ...
        private void RecalculateDisplayOrderFromVisualOrder()
        {
            ...
            for (int i = 0; i < _rows.Count; i++)
            {
                ...
                if (element.Pawn != null && element.Pawn.playerSettings != null)
                {
                    ...
                    BetterWorkTabMod.DebugLog($"  Row {i}: {PawnCompat.NameShortColored(element.Pawn)} â†’ displayOrder {i}", DebugFeature.DragDrop);
                }
            ...
            try
            {
                ...
                if (_snapshotPawns != null)
                {
                    for (int i = 0; i < _snapshotPawns.Count; i++)
                    {
                        ...
                        desired = Mathf.Max(desired, Text.CalcSize(PawnCompat.LabelShortCap(pawn)).x + 24f);
                    }
                }
            }
            finally
