...
namespace Better_Work_Tab.Features.Rules.RuleBuilder2
{
    ...
    internal static class RuleBuilder2ConditionCatalog
    {
...
#if vAlpha4
                    return Tr("BWT_RuleBuilder2_ConditionText_Trait", string.IsNullOrEmpty(condition.DefName) ? Tr("BWT_RuleBuilder2_TraitFallback") : condition.DefName);
#else
...
#if vAlpha4
                    return Tr("BWT_RuleBuilder2_ConditionText_CapacityMinimum", string.IsNullOrEmpty(condition.DefName) ? Tr("BWT_RuleBuilder2_CapacityFallback") : condition.DefName, condition.FloatValue.ToString("0.##"));
#else
        ...
        private static string GetSkillLabel(RuleBuilder2Condition condition, WorkTypeDef targetWorkType)
        {
            ...
            string workTypeLabel = WorkTypeCompat.LabelShort(targetWorkType);
            return workTypeLabel.NullOrEmpty() ? Tr("BWT_RuleBuilder2_RelevantFallback") : workTypeLabel;
        }
...
#if vAlpha4
            return def == null ? defName : (def.label.NullOrEmpty() ? defName : def.label.CapitalizeFirst());
#else
