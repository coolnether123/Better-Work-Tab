...
namespace Better_Work_Tab.Features.Rules.RuleBuilder2
{
    internal sealed class RuleBuilder2Evaluator
    {
        ...
        internal static List<Pawn> GetCurrentPawns()
        {
            ...
            {
                if (pawn != null && !PawnCompat.IsDead(pawn) && seen.Add(pawn))
                {
                    ordered.Add(pawn);
                }
            }
        ...
        {
            return MapCompat.FreeColonists(MapCompat.CurrentMap)
                .Where(pawn => pawn != null && !PawnCompat.IsDead(pawn))
        ...
        {
            string targetLabel = WorkTypeCompat.WorkGiverLabelCap(workGiver);
            if (targetLabel.NullOrEmpty())
            {
                targetLabel = WorkTypeCompat.LabelShort(workType);
            }
            if (targetLabel.NullOrEmpty())
            {
                targetLabel = Tr("BWT_RuleBuilder2_TargetFallback");
            }
