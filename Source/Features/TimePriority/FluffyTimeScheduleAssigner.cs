...
namespace Better_Work_Tab.Features.TimePriority
{
    ...
    internal static class FluffyTimeScheduleAssigner
    {
        ...
        internal static void Close()
        {
            ...
            IsOpen = false;
            _lastInteractiveRect = Better_Work_Tab.RectCompat.Zero;
            NotifyLayoutChanged();
        }
        ...
        internal static bool TryDrawWorkTypeCell(Rect cellRect, Pawn pawn, WorkTypeDef workType)
        {
            ...
            try
            {
                Text.WordWrap = false;
                #if v1_1 || v1_0 || v0_19
                WidgetsWorkCompat.DrawWorkBoxFor(boxRect.x, boxRect.y, pawn, workType, Find.PlaySettings.useWorkPriorities);
                #else
                WidgetsWorkCompat.DrawWorkBoxFor(boxRect, pawn, workType);
                #endif
        ...
        internal static bool ApplyWorkGiverPriority(int pawnId, WorkGiverDef workGiver, int priority)
        {
            ...
            Pawn pawn = pawnId == TimePriorityTarget.GlobalPawnId
                ? null
                : PawnsFinderCompat.AllAlive.FirstOrDefault(candidate => candidate.thingIDNumber == pawnId);
        ...
        private static void DrawControlButtons(Rect wholeDayButton, Rect nowButton)
        {
            if (FluffyWorkTabGateway.TryGetIcon(FluffyWorkTabIcon.PrioritiesWholeDay, out Texture2D wholeDayIcon) &&
                Better_Work_Tab.WidgetsCompat.ButtonImage(wholeDayButton, wholeDayIcon, Color.white, GenUI.MouseoverColor))
            {
                SelectWholeDay();
            }
            ...
            if (FluffyWorkTabGateway.TryGetIcon(FluffyWorkTabIcon.Now, out Texture2D nowIcon) &&
                Better_Work_Tab.WidgetsCompat.ButtonImage(nowButton, nowIcon, Color.white, GenUI.MouseoverColor))
            {
                SelectHour(MapCompat.CurrentMap != null ? TimeCompat.HourOfDay(MapCompat.CurrentMap) : 0, replace: true);
            }
