...
namespace Better_Work_Tab.Features.TimePriority
{
    ...
    internal static class TimePriorityPlannerPrototype
    {
        ...
        private const float InlineDividerHeight = 24f;
        ...
        {
            FinishCloseIfComplete();
            if (!IsEnabled || _session == null || layout == null || Event.current.type != EventType.Repaint)
            {
                return;
            }
        ...
        private static void DrawInlineEditor(IWorkTabLayoutController layout, WorkTabLayoutColumn sourceColumn, List<RowDrawInfo> rows, float progress)
        {
            ...
            for (int i = 0; i < rows.Count; i++)
            {
                combined = Union(combined, GetInlineTimelineRect(rows[i].RowRect, timelineX, timelineWidth));
            }

            _lastPanelRect = combined.ExpandedBy(2f);
            TooltipHandler.ClearTooltipsFrom(_lastPanelRect);

            DrawInlineDivider(dividerRect, timelineHeaderRect, timelineHeaderVisibleRect, progress);
        ...
        private static void DrawInlineDivider(Rect dividerRect, Rect timelineRect, Rect visibleTimelineRect, float progress)
        {
            Color oldColor = GUI.color;
            ...
            DrawInlineHourLabels(timelineRect, visibleTimelineRect, progress);
        ...
        private static void DrawInlineHourLabels(Rect timelineRect, Rect visibleTimelineRect, float progress)
        {
            ...
            for (int hour = 0; hour < HoursPerDay; hour++)
            {
                Rect labelRect = new Rect(timelineRect.x + hour * hourWidth, timelineRect.y + 4f, hourWidth, 14f);
        ...
        private static void DrawInlinePawnTimelineRow(IWorkTabLayoutController layout, RowDrawInfo row, float timelineX, float timelineWidth, float progress)
        {
            ...
            DrawInlineTimelineCells(row.Pawn, timelineRect, visibleTimelineRect, progress);
        ...
        private static void DrawInlineTimelineCells(Pawn pawn, Rect timelineRect, Rect visibleTimelineRect, float progress)
        {
            ...
            for (int hour = 0; hour < HoursPerDay; hour++)
            {
                ...
                DrawSchedulePriorityCell(hourRect, priorities[hour], hour, cellProgress);
        ...
        private static void DrawSchedulePriorityCell(Rect rect, int priority, int hour, float progress)
        {
            if (progress <= 0.001f)
            {
                return;
