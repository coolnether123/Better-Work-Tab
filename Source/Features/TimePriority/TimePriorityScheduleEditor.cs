...
namespace Better_Work_Tab.Features.TimePriority
{
    ...
    internal static class TimePriorityScheduleEditor
    {
        ...
        internal static bool OpenForPriorityBox(TimePriorityTarget target, Rect priorityBoxRect, int currentPriority)
        {
            ...
            _closingSourceRect = Better_Work_Tab.RectCompat.Zero;
        ...
        private static void CopySchedule(CopyPasteHit hit)
        {
            ...
            MessageCompat.Message("Copied " + hit.Label + " time priorities.", MessageTypeDefOf.PositiveEvent, false);
        }
        ...
        private static void PasteSchedule(CopyPasteHit hit)
        {
            ...
            MessageCompat.Message("Pasted " + snapshot.Label + " time priorities.", MessageTypeDefOf.PositiveEvent, false);
        }
        ...
        {
            StartCloseAnimation(_lastCloseRect != Better_Work_Tab.RectCompat.Zero ? _lastCloseRect : _session?.SourceBoxRect ?? Better_Work_Tab.RectCompat.Zero);
        }
        ...
        private static void StartCloseAnimation(Rect sourceRect)
        {
            ...
            _closingSourceRect = sourceRect != Better_Work_Tab.RectCompat.Zero ? sourceRect : _session.SourceBoxRect;
            NotifyLayoutChanged();
        }
        ...
        private static void FinishClose()
        {
            ...
            _lastPanelRect = Better_Work_Tab.RectCompat.Zero;
            _lastTimelineRect = Better_Work_Tab.RectCompat.Zero;
            _lastCloseRect = Better_Work_Tab.RectCompat.Zero;
            NotifyLayoutChanged();
        }
        ...
        internal static void AppendScheduleGeometryDiagnostics(System.Text.StringBuilder builder)
        {
            ...
            builder.AppendLine("timePrioritySourceRect=" + FormatRect(_session?.SourceBoxRect ?? Better_Work_Tab.RectCompat.Zero));
            ...
            out Rect labelRect)
        {
            ...
            {
                cellRect = Better_Work_Tab.RectCompat.Zero;
                boxRect = Better_Work_Tab.RectCompat.Zero;
                labelRect = Better_Work_Tab.RectCompat.Zero;
                return false;
            }
        ...
        private static bool TryGetPriorityTarget(IWorkTabLayoutController layout, Vector2 mousePosition, out TargetInfo target)
        {
            ...
            target = TargetInfo.ForWorkType(
                row.Pawn,
                workType,
                WorkTypeCompat.LabelShort(workType).CapitalizeFirst(),
                priorityBoxRect,
                priority);
            return true;
        }
            ...
            out TargetInfo target)
        {
            ...
            for (int i = 0; i < layout.Rows.Count; i++)
            {
                ...
                if (row.Pawn == null ||
                    PawnCompat.IsDead(row.Pawn) ||
                    PawnCompat.WorkSettings(row.Pawn) == null ||
                    !PawnCompat.HasEverWork(row.Pawn))
                {
                    continue;
                }
        ...
        private static void DrawHighlights(IWorkTabLayoutController layout, WorkTabLayoutColumn column, List<RowDrawInfo> rows, float progress)
        {
            ...
            Better_Work_Tab.WidgetsCompat.DrawBoxSolid(columnRect, columnColor);
        ...
        private static void DrawInlineEditor(IWorkTabLayoutController layout, WorkTabLayoutColumn sourceColumn, List<RowDrawInfo> rows, float progress)
        {
            ...
            Rect priorityRowsRect = Better_Work_Tab.RectCompat.Zero;
            for (int i = 0; i < rows.Count; i++)
            {
        ...
        {
            dividerRect = Better_Work_Tab.RectCompat.Zero;
            if (layout?.Rows == null)
            {
                return false;
            }
            ...
            float progress)
        {
            ...
            Better_Work_Tab.WidgetsCompat.DrawBoxSolid(visibleTimelineRect, GUI.color);
            ...
            Rect chronosRect = drawChronos ? GetInlineChronosRect(timelineRect, dividerProgress) : Better_Work_Tab.RectCompat.Zero;
        ...
        private static void DrawInlineDaylightBand(Rect timelineRect, float progress)
        {
            ...
            for (int hour = 0; hour < HoursPerDay; hour++)
            {
                ...
                Better_Work_Tab.WidgetsCompat.DrawBoxSolid(new Rect(bandRect.x + hour * hourWidth, bandRect.y, Mathf.Max(1f, hourWidth - 0.5f), bandRect.height), color);
            }
        }
        ...
        private static void DrawInlinePawnTimelineRow(IWorkTabLayoutController layout, RowDrawInfo row, float timelineX, float timelineWidth, float progress)
        {
            ...
            Better_Work_Tab.WidgetsCompat.DrawBoxSolid(visibleTimelineRect, new Color(0.035f, 0.045f, 0.05f, 0.95f * progress));
        ...
        private static string GetScheduleLabel(Pawn pawn)
        {
            ...
            return pawn == null ? target + " global" : PawnCompat.LabelShortCap(pawn) + " " + target;
        }
        ...
        {
            return _isClosing && _closingSourceRect != Better_Work_Tab.RectCompat.Zero
                ? _closingSourceRect
            ...
            for (int hour = 0; hour < HoursPerDay; hour++)
            {
                Color color = hour >= 7 && hour <= 18
                    ...
                    : hour == 6 || hour == 19 || hour == 20
                        ? new Color(0.48f, 0.43f, 0.64f, 0.62f * progress)
                        : new Color(0.14f, 0.18f, 0.30f, 0.70f * progress);
                Better_Work_Tab.WidgetsCompat.DrawBoxSolid(new Rect(bandRect.x + hour * hourWidth, bandRect.y, hourWidth, bandRect.height), color);
            }
        }
        ...
        private static void DrawPawnTimelineRow(Rect panelRect, RowDrawInfo row, int rowIndex, float progress)
        {
            ...
            Widgets.Label(new Rect(rowRect.x, rowRect.y + 2f, PawnLabelWidth - 6f, rowRect.height), PawnCompat.LabelShortCap(row.Pawn));
