...
namespace Better_Work_Tab.Patches
{
    ...
    public static class Patch_WorkPriority_DoCell_Unified
    {
        ...
        private static void DrawParentPriorityCellVisual(Rect rect, Pawn pawn, WorkTypeDef workType, float alpha)
        {
            ...
            if (alpha <= 0.001f || pawn == null || workType == null || workSettings == null)
            {
                return;
            }

            Rect boxRect = GetWorkBoxRect(rect);
            int priority = WorkPrioritySystem.ClampPriority(workSettings.GetPriority(workType));
            ...
            try
            {
                ...
                else if (priority > WorkPrioritySystem.DisabledPriority)
                {
                    ...
                    Better_Work_Tab.UI.WorkGiverReassignments.WorkGiverPriorityBoxCompatibility.DrawCheck(boxRect);
                }
            }
            finally
            {
