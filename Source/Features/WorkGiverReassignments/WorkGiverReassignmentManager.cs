...
namespace Better_Work_Tab.Features.WorkGiverReassignments
{
    ...
    internal static class WorkGiverReassignmentManager
    {
        ...
        private static List<WorkGiver> GetWorkGiversForWorkType(WorkTypeDef workType, Pawn pawn, bool applyPrioritySort)
        {
            ...
            {
                var worker = CreateWorker(remaining[i]);
                if (worker != null)
                {
                    result.Add(worker);
                }
        ...
        internal static bool HasNonEmergencyWorkGiver(WorkTypeDef workType)
        {
            ...
            return workType != null &&
                !workType.emergency &&
                WorkTypeCompat.WorkGiversByPriority(workType).Count > 0;
#else
        ...
        private static void ApplyEnableParentWorkType(int pawnId, string workTypeDefName)
        {
            ...
            var workType = DefDatabase<WorkTypeDef>.GetNamedSilentFail(workTypeDefName);
            Pawn_WorkSettings workSettings = PawnCompat.WorkSettings(pawn);
            if (workSettings == null || workType == null || pawn.WorkTypeIsDisabled(workType))
            {
                return;
            }
            ...
            WorkPrioritySystem.SetPriority(workSettings, workType, WorkPrioritySystem.GetDefaultEnabledPriority());
            NotifySubWorkDataChanged();
        }
        ...
        private static void ApplyEnableParentAndSetOnlySubOverride(int pawnId, string workTypeDefName, string workGiverDefName, int priority)
        {
            ...
            var workGiver = DefDatabase<WorkGiverDef>.GetNamedSilentFail(workGiverDefName);
            Pawn_WorkSettings workSettings = PawnCompat.WorkSettings(pawn);
            if (workSettings == null || workType == null || workGiver == null || pawn.WorkTypeIsDisabled(workType))
            {
                return;
            }
        ...
        private static void ApplyMoveWithinWorkType(string workTypeDefName, string workGiverDefName, int newIndex, int pawnId)
        {
            ...
            Pawn pawn = pawnId >= 0 ? FindPawnById(pawnId) : null;
        ...
        private static WorkGiver CreateWorker(WorkGiverDef def)
        {
            if (def == null)
            {
                return null;
