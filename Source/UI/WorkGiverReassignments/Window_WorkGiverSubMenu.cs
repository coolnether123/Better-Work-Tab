...
namespace Better_Work_Tab.UI.WorkGiverReassignments
{
    ...
    internal class Window_WorkGiverSubMenu : Window
    {
        ...
        public override Vector2 InitialPosition => new Vector2(
            (Verse.UI.screenWidth - InitialSubMenuSize.x) / 2f,
            (Verse.UI.screenHeight - InitialSubMenuSize.y) / 2f);
        ...
        {
            Text.Font = GameFont.Small;
            string workTypeLabel = WorkTypeCompat.LabelShort(_workType).CapitalizeFirst();
        ...
        private bool DrawVanillaHeader(WorkGiver wg, string label, Rect headerRect, bool isMovedFromBaseline, int level)
        {
            ...
            Rect hoverRect = RectCompat.ExpandedBy(textRect, 2f);
        ...
        private string BuildWorkGiverTooltip(WorkGiver wg)
        {
            ...
            string workType = WorkTypeCompat.LabelShort(wg.def.workType ?? _workType).CapitalizeFirst();
        ...
        private void ShowPawnDropdown(List<Pawn> pawns)
        {
            ...
            else
            {
                ...
                {
                    var localP = p;
                    options.Add(new FloatMenuOption(PawnCompat.LabelShortCap(localP), () => {
