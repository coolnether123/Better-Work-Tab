    ...
    internal class WorkGiverDragHandler
    {
            ...
            List<WorkGiver> workGivers, WorkGiverBaselineTracker baselineTracker)
        {
            ...
            if (_isDragging)
            {
                ...
                {
                    targetBox = RectCompat.ExpandedBy(hoveredRect.Value, 3f);
                }
                else
                {
            ...
            WidgetsCompat.DrawBoxSolid(new Rect(lineX, lineY, 1f, actualLineHeight), Color.white);
            ...
            if (_draggedIndex >= 0 && _draggedIndex < workGivers.Count)
            {
                ...
                if (targetBaselinePos >= 0)
                {
                    ...
                    WidgetsCompat.DrawBoxSolid(new Rect(baselineX, lineY, 1f, actualLineHeight), baselineColor);
                }
            }
        }
        ...
        private static void DrawBoxSolidWithOutline(Rect rect, Color fill, Color outline)
        {
            ...
            WidgetsCompat.DrawBoxSolid(rect, fill);
            GUI.color = outline;
