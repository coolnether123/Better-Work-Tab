...
namespace Better_Work_Tab.UI.RuleBuilderV2
{
    internal sealed class RuleBuilder2ConditionsView
    {
        ...
        internal void DrawConditionRow(Rect rect, RuleBuilder2Card card, RuleBuilder2Condition condition, int index)
        {
            ...
            Widgets.Checkbox(new Vector2(row.Checkbox.x, row.Checkbox.y), ref condition.Enabled);
...
#else
                    DrawFittedLabel(new Rect(rect.x, rect.y, 180f, rect.height), condition.DefName.NullOrEmpty() ? T("BWT_SelectTrait") : condition.DefName);
#endif
                    break;
...
#else
                    DrawFittedLabel(new Rect(rect.x, rect.y, 220f, rect.height), T("BWT_RuleBuilder2_CapacityFallback"));
#endif
                    break;
        ...
        internal void ShowSkillMenu(RuleBuilder2Condition condition, WorkTypeDef workType)
        {
            var options = RuleBuilder2ConditionCatalog.GetSkillOptions(workType)
                .Select(skill => new FloatMenuOption(SkillLabel(skill), () => condition.DefName = skill.defName))
                .ToList();
            Find.WindowStack.Add(new FloatMenu(options));
        }
...
#if vAlpha4
            return def == null ? condition.DefName : (def.label.NullOrEmpty() ? condition.DefName : def.label.CapitalizeFirst());
#else
        ...
        internal static void ShowDefMenu<T>(RuleBuilder2Condition condition, int maxOptions = 80) where T : Def, new()
        {
            var options = DefDatabase<T>.AllDefsListForReading
                ...
                .Take(maxOptions)
#if vAlpha4
                .Select(def => new FloatMenuOption(def.label.NullOrEmpty() ? def.defName : def.label.CapitalizeFirst(), () => condition.DefName = def.defName))
#else
        ...
        private string SkillLabel(SkillDef skill)
        {
            if (skill == null)
            {
                return T("BWT_RuleBuilder2_SkillFallback");
            }

#if vAlpha4
            return skill.label.NullOrEmpty() ? skill.defName : skill.label.CapitalizeFirst();
#else
