...
namespace Better_Work_Tab.ModSupport
{
    internal static class CompatibilityDiagnostics
    {
        ...
        {
            List<ModContentPack> mods = GetRunningMods();
            for (int i = 0; i < mods.Count; i++)
            {
                ...
                if (ContainsIgnoreCase(modName, "work tab") || ContainsIgnoreCase(modPackageId, "worktab"))
                {
                    WarningOnce(
                        $"[Better Work Tab] Possible Work tab UI mod also active: {modName} ({modPackageId}). " +
        ...
        private static void ReportKnownMultiplayerLoadOrderRisk(ModContentPack betterWorkTab)
        {
            ...
            for (int i = 0; i < mods.Count; i++)
            {
                ...
                string modPackageId = GetPackageId(mod);
                string modName = GetModName(mod);
                if (ContainsIgnoreCase(modName, "rimhammer") || ContainsIgnoreCase(modPackageId, "rimhammer"))
                {
            ...
            WarningOnce(
                $"[Better Work Tab] Multiplayer + HugsLib + Rimhammer detected, with BWT loading before {GetModName(lastRimhammer)} ({GetPackageId(lastRimhammer)}). " +
        ...
        private static void ErrorOnce(string message, int key)
        {
            if (!ReportedWarnings.Add(key))
            {
                return;
        ...
        private static List<ModContentPack> GetRunningMods()
        {
            Type type = typeof(LoadedModManager);
            string[] propertyNames = { "RunningModsListForReading", "RunningMods" };
            for (int i = 0; i < propertyNames.Length; i++)
            {
                PropertyInfo property = type.GetProperty(propertyNames[i], BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                if (property == null)
                {
                    continue;
                }
                ...
                if (value is IEnumerable<ModContentPack> mods)
                {
                    return mods.Where(mod => mod != null).ToList();
                }
            }

            FieldInfo field = type.GetField("runningMods", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
            if (field != null)
            {
        ...
        private static string GetModName(ModContentPack mod)
        {
            if (mod == null)
            {
                return string.Empty;
