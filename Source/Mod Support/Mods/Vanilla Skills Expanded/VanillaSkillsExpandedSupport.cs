...
namespace Better_Work_Tab.ModSupport
{
    ...
    public sealed class VanillaSkillsExpandedSupport : ModSupportModuleBase
    {
        ...
        public static bool PassionMatches(Pawn_SkillTracker skills, WorkTypeDef workType, int requiredPassion)
        {
            ...
            Passion actual = MaxPassionOfRelevantSkillsFor(skills, workType);
        ...
        private static void Refresh(bool force)
        {
            ...
            try
            {
                ...
                foreach (object rawDef in defs)
                {
                    ...
                    int passionValue = ReadDefIndex(def, rawDef, reflected.Count);
                    float learnRateFactor = ReadField(learnRateFactorField, rawDef, GetVanillaLearnRate(passionValue));
        ...
        private static int ReadDefIndex(Def def, object rawDef, int fallback)
        {
#if vAlpha4
            if (rawDef == null)
            {
            ...
            FieldInfo indexField = rawDef.GetType().GetField("index", flags);
            if (indexField != null && indexField.GetValue(rawDef) is int fieldValue)
            {
                return fieldValue;
            }

            PropertyInfo indexProperty = rawDef.GetType().GetProperty("index", flags);
            if (indexProperty != null && indexProperty.GetValue(rawDef, null) is int propertyValue)
            {
