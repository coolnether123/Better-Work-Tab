...
namespace Better_Work_Tab.ModSupport
{
    ...
    public sealed class DoOnceSupport : ModSupportModuleBase
    {
        ...
        private static string GetModName(ModContentPack mod)
        {
            ...
            Type type = mod.GetType();
            foreach (string memberName in new[] { "Name", "name", "NameReadable" })
            {
                PropertyInfo property = type.GetProperty(memberName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                if (property != null)
                {
                    object propertyValue = property.GetValue(mod, null);
                    if (propertyValue is string propertyString && !string.IsNullOrEmpty(propertyString))
                    {
                ...
                if (field != null)
                {
                    object fieldValue = field.GetValue(mod);
