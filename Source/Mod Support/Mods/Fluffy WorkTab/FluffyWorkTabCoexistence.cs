...
namespace Better_Work_Tab.ModSupport.Mods.FluffyWorkTab
{
    ...
    internal static class FluffyWorkTabCoexistence
    {
...
#if v0_18
                AccessTools.Field(typeof(MainButtonDef), "tabWindowInt")?.SetValue(work, null);
#else
        ...
        private static void EnsureDetected()
        {
            ...
            IEnumerable<ModContentPack> mods = runningModsField?.GetValue(null) as IEnumerable<ModContentPack>;
#else
            IEnumerable<ModContentPack> mods = LoadedModManager.RunningModsListForReading;
#endif
            if (mods == null)
            {
                return;
